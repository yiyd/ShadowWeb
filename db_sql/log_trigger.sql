...
create trigger t_users_update
	...
	begin
		...
		if (NEW.user_name <> OLD.user_name) then
			insert into admin_log_fields values ('', '用户名', OLD.user_name, NEW.user_name);
		end if;
		if (NEW.user_passwd <> OLD.user_passwd) then
			insert into admin_log_fields values ('', '密码', OLD.user_passwd, new.user_passwd);
		end if;
		if (new.role_id <> OLD.role_id) then
			insert into admin_log_fields values ('', '角色', old.role_id, new.role_id);
		end if;
		if (new.user_mail <> old.user_mail) then
			insert into admin_log_fields values ('', '邮箱', old.user_mail, new.user_mail);		
		end if;
	end;
//
...
create trigger t_roles_update
	...
	begin
		...
		if (new.role_name <> old.role_name) then
			insert into admin_log_fields values ('', '角色名', old.role_name, new.role_name);
		end if;
	end;
//
	...
	begin 
		insert into admin_logs values ('', CURRENT_TIMESTAMP, '修改权限', new.priv_id);
			if (new.priv_name <> old.priv_name) then
				insert into admin_log_fields values ('', '权限名', old.priv_name, new.priv_name);
			end if;
			if (new.priv_value <> old.priv_value) then
				insert into admin_log_fields values ('', '权限值', old.priv_value, new.priv_value);
			end if;
	end;
//
...
create trigger t_role_priv_insert 
	...
	begin
		...
		insert into admin_log_fields values ('', '权限ID', 'null', new.priv_id);
	end;
//
...
create trigger t_role_priv_delete
	...
	begin
		...
		insert into admin_log_fields values ('', '权限ID', old.priv_id, 'null');
	end;
//
